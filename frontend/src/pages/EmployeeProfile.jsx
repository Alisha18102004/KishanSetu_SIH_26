...
export default function EmployeeProfile({ user, onBack, onUserUpdate }) {
  ...
  const Field = ({ icon: Icon, label, value, name, type = "text" }) => (
    editing ? (
      <label className="block">
        <span className="mb-1.5 block text-xs font-bold text-slate-500">{label}</span>
        <div className="relative">
          <Icon size={17} className="absolute left-3 top-3.5 text-emerald-700" />
          <input
            className="ks-input w-full pl-10"
            type={type}
            value={form[name] ?? ""}
      ...
      <Card className="overflow-hidden">
        ...
        <div className="p-5 md:p-7">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Field icon={Building2} label={labels.name} value={user?.name} name="name" />
            <Field icon={MapPin} label={labels.district} value={user?.district} name="district" />
            <Field icon={MapPin} label={labels.city} value={user?.city} name="city" />
            <Field icon={UsersRound} label={labels.counters} value={user?.counters} name="counters" type="number" />
            <div className="md:col-span-2"><Field icon={MapPin} label={labels.address} value={user?.address} name="address" /></div>
          </div>
