  ...
  return <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
    <div className="mx-auto flex h-[70px] max-w-7xl items-center justify-between px-4 md:px-7">
      <div className="flex items-center gap-3">
        ...
        <div><div className="text-[19px] font-black leading-none text-emerald-800">KisanSetu</div><div className="mt-1 text-[11px] font-semibold text-slate-500">Smart Mandi Services</div></div>
      </div>
      <div className="flex items-center gap-2 rounded-full bg-indigo-50 px-3.5 py-2 text-xs font-bold text-slate-700 md:flex"><Languages size={17} className="text-emerald-700" /><button type="button" onClick={() => setLanguage(language === "hi" ? "en" : "hi")} className="font-bold" title={language === "hi" ? "Switch to English" : "हिंदी में बदलें"}>{language === "hi" ? "हिंदी → EN" : "EN → हिंदी"}</button></div>
      <div className="flex items-center gap-2">
        ...
        <button type="button" onClick={onProfile} className="hidden max-w-[180px] truncate rounded-full bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-800 hover:bg-emerald-100 sm:block">{user?.name || user?.centre_id || "User"}</button>
